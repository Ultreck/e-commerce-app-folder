...
import DeliverOrderComponent from '@/components/DeliverOrderComponent'
      ...
      <div className="text">
        <div className="text">
          <ul className="text flex items-center justify-between w-full p-5 ">
            {["All orders", "Processing", "Shipped", "Delivered", "Returns"].map((tab, index) => (
              <li  
              onClick={() => setTabIndex(index)} 
              key={tab} 
              className="text-sm "
              >
                <Link className={`px-5 font-semibold py-1 transform duration-200 ${index === tabIndex? "border-b-2 border-amber-700 text-black" : "text-gray-400"}`} href={'#'}>{tab}</Link>
              </li>
            ))
            }
          </ul>
